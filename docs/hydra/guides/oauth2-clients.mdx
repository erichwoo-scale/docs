...
you control, for example, backends and services, are confidential clients. They can authenticate with a client secret or a signed
JSON Web Token. Self-hosted Ory Hydra can also
[authenticate them with a client certificate](../self-hosted/mtls-client-authentication.mdx). Applications distributed to users,
for example, single-page apps, mobile, and native apps, are public clients. Anyone can read the code and extract an embedded
secret, so they authenticate with PKCE and have no secret at all.

For each of your confidential clients, register a client record with the authentication material required by its
`token_endpoint_auth_method`. Public clients don't have a client secret in the registered client record and use `none` as their
