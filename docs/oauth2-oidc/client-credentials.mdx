...
flow, the application sends its client ID and configured authentication credentials to the authorization server and receives an
access token that it can use to access protected resources.
...
:::note Self-hosted Ory Hydra

In addition to the methods on this page, self-hosted Ory Hydra supports certificate-based client authentication with
`tls_client_auth` and `self_signed_tls_client_auth`. To configure the reverse proxy, Ory Hydra, and clients, follow the
[mutual TLS client authentication guide](../hydra/self-hosted/mtls-client-authentication.mdx).
